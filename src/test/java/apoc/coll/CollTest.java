...
public class CollTest {
    ...
    public void testDropNeighboursNodes() throws Exception {
        ...
        testResult(db, "MATCH p=(n)-[:KNOWS]->(m)-[:LIVES_WITH]->(h) CALL apoc.coll.dropDuplicateNeighbors(nodes(p)) yield value return value",
                (row) -> {
    ...
    public void testDropNeighboursNumbers() throws Exception {
        testResult(db, "WITH [1,2,3,4,4,5,6,6,4,7] AS values CALL apoc.coll.dropDuplicateNeighbors(values) yield value return value",
                (row) -> {
    ...
    public void testDropNeighboursStrings() throws Exception {
        testResult(db, "WITH ['a','a','hello','hello','hello','foo','bar','apoc','apoc!','hello'] AS values CALL apoc.coll.dropDuplicateNeighbors(values) yield value return value",
                (row) -> {
    ...
    public void testDropNeighborsWithDifferentValueTypesFail() {
        try {
            testCall(db, "WITH [1,2,3,'hello',4,5,6,6,'a','b'] AS values CALL apoc.coll.dropDuplicateNeighbors(values) yield value return value", (r) -> {});
        } catch (IllegalArgumentException e) {
            Throwable except = ExceptionUtils.getRootCause(e);
            assertTrue(except instanceof IllegalArgumentException);
            assertEquals("List elements must be the same type", except.getMessage());
            throw e;
        }
