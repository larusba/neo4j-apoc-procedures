...
public class Json {
    ...
    public Stream<MapResult> toTree(@Name("paths") List<Path> paths, @Name(value = "lowerCaseRels",defaultValue = "true") boolean lowerCaseRels, @Name(value = "config", defaultValue = "{}") Map<String, Object> config) {
        ...
        Map<String, List<String>> nodePropertyFilter = conf.getNodes();
        Map<String, List<String>> relationshipPropertyFilter = conf.getRelationships();
        ...
        for (Path path : paths) {
            ...
            while (it.hasNext()) {
                Node n = (Node) it.next();
                Map<String, Object> nMap = maps.computeIfAbsent(n.getId(), (id) -> toMap(n, nodePropertyFilter));
                if (it.hasNext()) {
                    ...
                    Optional<Map<String, Object>> optMap = list.stream()
                            ...
                            .findFirst();
                    if (!optMap.isPresent()) {
                        Map<String, Object> mMap = toMap(m, nodePropertyFilter);
                        mMap = addRelProperties(mMap, typeName, r, relationshipPropertyFilter);
