...
public class Schemas {
    ...
    private Stream<IndexConstraintNodeInfo> indexesAndConstraintsForNode(Map<String,Object> config) throws IndexNotFoundKernelException {
        ...
        Collection<String> includeLabels = schemaConfig.getLabels();
        Collection<String> excludeLabels = schemaConfig.getExcludeLabels();
    ...
    private Stream<ConstraintRelationshipInfo> constraintsForRelationship(Map<String,Object> config) {
        ...
        SchemaConfig schemaConfig = new SchemaConfig(config);
        Collection<String> includeRelationships = schemaConfig.getRelationships();
        Collection<String> excludeRelationships = schemaConfig.getExcludeRelationships();
    ...
    private Stream<GraphResult> filterResultStream(Set<String> excludes, Stream<GraphResult> graphResultStream) {
        ...
        return graphResultStream.map(gr -> {
            Iterator<Node> nodeIterator = gr.nodes.iterator();
            while (nodeIterator.hasNext()) {
                Node node = nodeIterator.next();
                if (containsLabelName(excludes,node)) nodeIterator.remove();
            }

            Iterator<Relationship> relationshipIterator = gr.relationships.iterator();
