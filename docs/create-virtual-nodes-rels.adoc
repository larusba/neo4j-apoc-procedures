...
CALL apoc.nodes.collapse(subgraph,{properties:'combine', mergeVirtualRels:true, countMerge: true}) yield from, rel, to
return from, rel, to
----
we have this result:
...
CALL apoc.nodes.collapse(subgraph,{mergeVirtualRels:true, countMerge: true}) yield from, rel, to
return from, rel, to
----
we have this result:
