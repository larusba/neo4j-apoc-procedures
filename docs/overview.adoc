...
[cols="1m,5"]
|===
| apoc.text.indexOf(text, lookup, offset=0)| find the first occurence of the lookup string in the text, if offset is given then start from there, -1 if not found, null if text is null.
