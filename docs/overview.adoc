...
| CALL apoc.load.jdbcUpdate(jdbc-url,statement, params) | update relational database from a SQL kernelTransaction with optional parameters
|===
...
For load file from compressed file the url right sintax is:
...
| apoc.nodes.collapse([<node>], {config}) | returns the list of nodes merged into the first
...
On `apoc.nodes.collapse` there are many config param:
 * "mergeRels: true/false" : give the possibility to merge relationships with same type and direction.
 * "selfRel: true/false" : give the possibility to create the self relationship.
 * "countProperties: true/false" : give the possibility count all the properties merged for nodes and relationships.
...
CALL apoc.nodes.collapse(subgraph,{properties:'combine', mergeRels:true, countProperties: true}) yield from, rel, to
return from, rel, to
----
we have this result:
...
CALL apoc.nodes.collapse(subgraph,{properties:'ignore', mergeRels:true, countProperties: true}) yield from, rel, to
return from, rel, to
----
we have this result:
